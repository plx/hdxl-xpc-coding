...
struct ErrorConditionTests {
  ...
  // MARK: - XPCDecoder Initialization

  @Test("XPCDecoder rejects XPC array in initializer", .tags(.decoding))
  func decoderRejectsArrayInitialization() throws {
    let array = xpc_array_create(nil, 0)

    #expect(throws: DecodingError.self) {
      try XPCDecoder(decoding: array)
    }

    // Verify it's specifically a dataCorrupted error
    do {
      _ = try XPCDecoder(decoding: array)
      Issue.record("Expected DecodingError to be thrown")
    } catch let error as DecodingError {
      guard case .dataCorrupted(let context) = error else {
        Issue.record("Expected dataCorrupted error, got \(error)")
      ...
      #expect(context.codingPath.isEmpty)
      #expect(context.debugDescription.contains("non-dictionary"))
      #expect(context.debugDescription.contains("array"))
    }
  }

  @Test("XPCDecoder rejects XPC string in initializer", .tags(.decoding))
  func decoderRejectsStringInitialization() throws {
    let string = "test".withCString { xpc_string_create($0) }

    #expect(throws: DecodingError.self) {
      try XPCDecoder(decoding: string)
    }

    // Verify it's specifically a dataCorrupted error
    do {
      _ = try XPCDecoder(decoding: string)
      Issue.record("Expected DecodingError to be thrown")
    } catch let error as DecodingError {
      guard case .dataCorrupted(let context) = error else {
        Issue.record("Expected dataCorrupted error, got \(error)")
  ...
  @Test("XPCDecoder rejects XPC int64 in initializer", .tags(.decoding))
  func decoderRejectsInt64Initialization() throws {
    let int64 = xpc_int64_create(42)

    #expect(throws: DecodingError.self) {
      try XPCDecoder(decoding: int64)
    }

    // Verify it's specifically a dataCorrupted error
    do {
      _ = try XPCDecoder(decoding: int64)
      Issue.record("Expected DecodingError to be thrown")
    } catch let error as DecodingError {
      guard case .dataCorrupted(let context) = error else {
        Issue.record("Expected dataCorrupted error, got \(error)")
      ...
      #expect(context.debugDescription.contains("non-dictionary"))
      #expect(context.debugDescription.contains("int64"))
    }
  }

  @Test("XPCDecoder rejects XPC bool in initializer", .tags(.decoding))
  func decoderRejectsBoolInitialization() throws {
    let bool = xpc_bool_create(true)

    #expect(throws: DecodingError.self) {
      try XPCDecoder(decoding: bool)
    }

    // Verify it's specifically a dataCorrupted error
    do {
      _ = try XPCDecoder(decoding: bool)
      Issue.record("Expected DecodingError to be thrown")
    } catch let error as DecodingError {
      guard case .dataCorrupted(let context) = error else {
        Issue.record("Expected dataCorrupted error, got \(error)")
      ...
      #expect(context.debugDescription.contains("non-dictionary"))
      #expect(context.debugDescription.contains("bool"))
    }
  }

  @Test("XPCDecoder accepts XPC dictionary in initializer", .tags(.decoding))
  func decoderAcceptsDictionaryInitialization() throws {
    let dict = xpc_dictionary_create(nil, nil, 0)

    // Should not throw
    let decoder = try XPCDecoder(decoding: dict)
